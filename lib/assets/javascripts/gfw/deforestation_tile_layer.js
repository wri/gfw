...
var Deforestation = function() {
  ...
  function filter(image_data, w, h, year_start, year_end) {
    ...
    for(var i=0; i < w; ++i) {
      for(var j=0; j < h; ++j) {
        ...
        var yearLoss = image_data[pixel_pos+2];
        var intensity = image_data[pixel_pos];
        ...
        if (yearLoss >= year_start && yearLoss < year_end) {
          ...
          if (zoom < 13) {
            intensity>0? image_data[pixel_pos+ 3] = intensity+t-(t*intensity)/256 : image_data[pixel_pos+ 3] =0;
          } else {
            image_data[pixel_pos+ 3] = intensity;
          }
        } else {
  ...
  function canvas_setup(canvas, coord, zoom) {
    ...
    switch (config.BASELAYER) {
      case 'loss75' : var url = 'http://earthengine.google.org/static/hansen_2013/gfw_tree_loss_year_75/' + z + '/' + x + '/' + y + '.png';
      break;
      case 'loss50' : var url = 'http://earthengine.google.org/static/hansen_2013/gfw_tree_loss_year_50/' + z + '/' + x + '/' + y + '.png';
      break;
      case 'loss30' : var url = 'http://earthengine.google.org/static/hansen_2013/gfw_tree_loss_year_30/' + z + '/' + x + '/' + y + '.png';
      break;
      case 'loss25' : var url = 'http://earthengine.google.org/static/hansen_2013/gfw_tree_loss_year_25/' + z + '/' + x + '/' + y + '.png';
      break;
      case 'loss20' : var url = 'http://earthengine.google.org/static/hansen_2013/gfw_tree_loss_year_20/' + z + '/' + x + '/' + y + '.png';
      break;
      case 'loss15' : var url = 'http://earthengine.google.org/static/hansen_2013/gfw_tree_loss_year_15/' + z + '/' + x + '/' + y + '.png';
      break;
      case 'loss10' :
      case 'loss' :
      default:         var url = 'http://earthengine.google.org/static/hansen_2013/gfw_tree_loss_year_10/' + z + '/' + x + '/' + y + '.png';
    }
  ...
  me.init = function(map, percent, callback) {
    this.map = map;
