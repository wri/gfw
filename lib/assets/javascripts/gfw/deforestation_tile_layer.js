...
var Deforestation = function() {
  ...
  function filter(image_data, w, h, year_start, year_end) {
    ...
    //---> logaritmic scale
    //var myscale=d3.scale.log().domain([1,256]).range([1,256]);
    ...
    for(var i=0; i < w; ++i) {
      for(var j=0; j < h; ++j) {
        ...
        if (yearLoss >= year_start && yearLoss < year_end) {
          ...
          //---> zoom & intensity dependent color 
          image_data[pixel_pos] = 220;
          image_data[pixel_pos + 1] = (72-zoom)+102-(3*myscale(intensity)/zoom);
          image_data[pixel_pos + 2] = (33-zoom)+153-((intensity)/zoom);


          if (zoom < 13) {
            //intensity>0? image_data[pixel_pos+ 3] = intensity+t-(t*intensity)/256 : image_data[pixel_pos+ 3] =0;
            //image_data[pixel_pos+ 3]=myscale(intensity);
            intensity>t? image_data[pixel_pos+ 3] =myscale(intensity): image_data[pixel_pos+ 3]=intensity;

            //hancher's
            //image_data[pixel_pos+ 3] = 255 * Math.pow(intensity / 255, 0.7);
            //image_data[pixel_pos+ 3] = 255 * Math.pow(intensity / 255, 1/zoom);
  ...
  function canvas_setup(canvas, coord, zoom) {
    ...
    if (config.canopy_choice) {
      ...
      var canopy = (config.BASELAYER === 'loss') ? '10' : config.BASELAYER.slice(-2);
  ...
  me.show = function() {
    this.heightLayer = new DeforestationTileLayerThreshold(canvas_setup, filter);
    this.map.overlayMapTypes.insertAt(this.map.overlayMapTypes.getLength(), this.heightLayer);
