...
var Deforestation = function() {
  ...
  function filter(image_data, w, h, year_start, year_end) {
    ...
    //---> zoom -dependent power scale
    var myscale=d3.scale.pow().exponent(1/zoom).domain([0,256]).range([0,256]);

    //---> logaritmic power scale
    //var myscale=d3.scale.log().domain([1,256]).range([1,256]);

    for(var i=0; i < w; ++i) {
      console.log(i, '->', myscale(i));
      for(var j=0; j < h; ++j) {
        ...
        if (yearLoss >= year_start && yearLoss < year_end) {
          ...
          image_data[pixel_pos + 2] = (33-zoom)+153-(myscale(intensity)/zoom);
