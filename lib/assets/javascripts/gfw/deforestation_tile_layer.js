...
var Deforestation = function() {
    ...
    for(var i=0; i < w; ++i) {
      for(var j=0; j < h; ++j) {
        ...
        if (yearLoss >= year_start && yearLoss <= year_end) {
          var c = (Math.round(3*intensity/255));
          var color = [255, 0, 0];//colors[Math.min(2, c)];
          image_data[pixel_pos] = 220;
          image_data[pixel_pos + 1] = 102;
          image_data[pixel_pos + 2] = 153;
          if (zoom > 8) {
            image_data[pixel_pos+ 3] = 255*c/3;//intensity < 1 ? 0: 255;
          } else {
  ...
  me.init = function(map, callback) {
    ...
    if (forest2000.attributes['visible'] && callback) callback();
