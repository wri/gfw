function IdnPrimaryTileLayer(canvas_setup, filter) {
  this.tileSize = new google.maps.Size(256, 256);
  this.maxZoom = 19;
  this.name = "idn_primary";
  this.alt = "Canvas idn_primary tile layer";
  this.tiles = {};
...
IdnPrimaryTileLayer.prototype.create_tile_canvas = function(coord, zoom, ownerDocument) {
...
IdnPrimaryTileLayer.prototype.filter_tile = function(canvas) {
  var ctx = canvas.getContext('2d');
...
IdnPrimaryTileLayer.prototype.composed = function(map, w, h) {
...
IdnPrimaryTileLayer.prototype.filter_tiles = function() {
  var args = Array.prototype.slice.call(arguments);
  for(var c in this.tiles) {
    this.filter_tile(this.tiles[c], args);
  }
}
IdnPrimaryTileLayer.prototype.getTile = function(coord, zoom, ownerDocument) {
  // could be called directly...
  return this.create_tile_canvas(coord, zoom, ownerDocument);
};

IdnPrimaryTileLayer.prototype.releaseTile = function(tile) {
  var id = tile.getAttribute('id');
  delete this.tiles[id];
};

// optimized version for threshold rendering
function IdnPrimaryTileLayerThreshold (canvas_setup, filter) {
  IdnPrimaryTileLayer.call(this, canvas_setup, filter);
  this.threshold = 0;
  this.start_threshold = 0;
}


IdnPrimaryTileLayerThreshold.prototype = new IdnPrimaryTileLayer();

IdnPrimaryTileLayerThreshold.prototype.filter_tiles = function() {
  var new_threshold = arguments[0];
  IdnPrimaryTileLayer.prototype.filter_tiles.apply(this, arguments)
  this.threshold = new_threshold;
}

IdnPrimaryTileLayerThreshold.prototype.filter_tile = function(canvas) {
  var ctx = canvas.getContext('2d');
...
var IdnPrimary = function() {
  ...
  function canvas_setup(canvas, coord, zoom) {
    ...
    var url = 'https://s3.amazonaws.com/idn/idnfc2/%z/%x/%y.png';
    ...
    xhr.onload = function () {
      ...
      image.onload = function () {
        ...
        IdnPrimary.heightLayer.filter_tile(canvas);
  ...
  me.init = function(map, callback) {
    this.map = map;
    this.heightLayer = new IdnPrimaryTileLayerThreshold(canvas_setup, filter);
  ...
  me.hide = function() {
    ...
    if (overlays_length > 0) {
      for (var i = 0; i< overlays_length; i++) {
        ...
        if (layer && layer.name == 'idn_primary') this.map.overlayMapTypes.removeAt(i);
      }
    }
  }
