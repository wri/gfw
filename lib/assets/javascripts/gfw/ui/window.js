gfw.ui.model.Window = Backbone.Model.extend({
...
gfw.ui.view.Window = gfw.ui.view.Widget.extend({
  ...
  add: function(id, category, category_title, title, category_color, title_color) {
    ...
    this.open(function() {
      ...
      if (!that.categories[category]) {
        ...
        that.categories[category] = new gfw.ui.collection.WindowItems;
        that.categories[category].bind("add", that.addContent);
      }

      var item = new gfw.ui.model.WindowItem({
        cat_id:         id,
        title:          title,
        category:       category,
