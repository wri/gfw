gfw.ui.model.TimelineModis = Backbone.Model.extend({
  defaults: {
    hidden: true,
    handle: false
  }
...
gfw.ui.view.TimelineModis = gfw.ui.view.Widget.extend({
  ...
  loadDefaultRange:function() {
    if (this.$el.find('.selected').length > 0) {
      ...
      this._updateMap($selected.attr('data-month'), $selected.attr('data-year'));
    } else {
      this._updateMap(12, 2013);
    }
  },
  ...
  _updateMap: function(month, year) {
    ...
    publish('timeline:change_modis_date', [month, year]);
  },
  ...
  _loadDefaultQuarter:function() {
    ...
    for(var i = 0; i <= yearCount; i++) {
      for(var j = 1; j <= quarters; j++) {
        var quarter = ( (j*4 <= 9) ? "0" + j*3 : j*3 );

        sql += [
                '(WITH modis as (SELECT the_geom ',
