...
gfw.ui.view.AnalysisInfo = gfw.ui.view.Widget.extend({
  ...
  initialize: function() {
    ...
    // TODO: Hack. :) Better way to get country ISO code?
    this.countryCode = window.location.href.split('/')[7];
    this.formaDownloadURL = 'http://gfw_downloads_iso.s3.amazonaws.com/' + this.countryCode + '.zip';
    ...
    subscribe('timeline:change_date', _.bind(function(y0, m0, y1, m1) {
      var start = '';
      var end = '';
      var url = 'http://gfw-apis.appspot.com/api/v1/defor/analyze/forma/iso'
      ...
      this.formaAlertsUrl = [url, iso, start, end].join('/');
      ...
      if((m0 >= 1 && m0 <= 12) && (m1 >= 1 && m1 <= 12)) {
        this.model.set({'subtitle': y0+", "+config.MONTHNAMES_SHORT[m0-1].charAt(0).toUpperCase() + config.MONTHNAMES_SHORT[m0-1].slice(1).toLowerCase()+" to "+y1+", "+config.MONTHNAMES_SHORT[m1-1].charAt(0).toUpperCase() + config.MONTHNAMES_SHORT[m1-1].slice(1).toLowerCase()});
      }
    }, this));

    // Fire off API request for FORMA alert count for current timeline dates:
    subscribe('timeline:onStopDragging', _.bind(function(y0, m0, y1, m1) {
      this._getAlertCount();
    }, this));
  },
    ...
    executeAjax(
      ...
      {}, 
      {
        success: _.bind(function(data) {
          this.model.set({'alert_count': data.value_display + ' alerts'});
        }, this), 
  ...
  show: function() {
    ...
    this.$('a.download').attr('href', this.formaDownloadURL);
  },
  ...
  render: function() {
    ...
    this.$alert_count = this.$el.find("#forma-count");
