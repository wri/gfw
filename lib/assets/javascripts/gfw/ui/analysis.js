...
gfw.ui.view.Analysis = gfw.core.View.extend({
  ...
  _loadProtectedArea: function() {

    this._loadProtectedAreaGeoJSON();
  ...
  _loadProtectedAreaGeoJSON: function(id) {
    ...
    var that = this;
    var query = "http://www.protectedplanet.net/sites/555538211/protected_areas/geom?simplification=0";
    ...
    $.ajax({
      ...
      var pa = pa.the_geom;
        ...
        var features = new GeoJSON(pa, style);
        ...
        for (var i in features) {
          if (features[i].length > 0) {
            for (var j in features[i]) {
              ...
              feature.setMap(that.map);
              that.selectedShapes.push(feature);
            }
          } else {
  ...
  _loadCountryGeoJSON: function(ISO) {
    ...
    $.ajax({
      ...
      success: function(pa) {
        ...
        var polygon = pa.features[0].geometry;
        console.log(polygon);
        console.log(that._calcArea(polygon));
        //var zoom = that.map.getZoom() - 1;
