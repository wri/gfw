...
gfw.ui.view.AnalysisInfo = gfw.ui.view.Widget.extend({
  ...
  initialize: function() {
    ...
    subscribe('timeline:change_date', _.bind(function(y0, m0, y1, m1) {
      ...
      if(GFW.app.currentBaseLayer == "brazilian_amazon") {
        this.end = '';
        this.iso = '';
      } else {
        this.end = end;
      ...
      this.url = 'http://gfw-apis.appspot.com/datasets/'
  ...
  _getAlertCount: function() {
    if (this.initStats) {
      ...
      if (this.model.get("dataset") === 'imazon') {
        polytype = polytype.replace(/\"MultiPolygon\"/i, "\"Polygon\"");
      } else {
        polytype = polytype.replace(/\"Polygon\"/i, "\"MultiPolygon\"");
    ...
    executeAjax(
      ...
      {}, 
      {
        success: _.bind(function(data) {
          if(data && data.value) {
            this.model.set({'alert_count': data.value + ' alerts'});
          } else {
