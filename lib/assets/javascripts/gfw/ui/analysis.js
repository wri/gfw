...
gfw.ui.view.AnalysisInfo = gfw.ui.view.Widget.extend({
  ...
  _getAlertCount: function() {
    ...
    if (!this.initStats) return;
      ...
      if (this.dataset == 'umd') {
        this.alertsUrl = 'http://wip.gfw-apis.appspot.com/datasets/hansen?layer=sum&geom=' + encodeURI(polytype) + '&bust=1';
      } else if (this.iso === 'ALL') {
        ...
        success: _.bind(function(data) {
          ...
          if (umdUpdated) {
            ...
            this.$el.find(".stats .spinner").fadeOut(250, function() {
              that.$el.find(".stats .hidden").fadeIn(250);
            });
          }
...
gfw.ui.view.DownloadDropdown = gfw.core.View.extend({
...
gfw.ui.view.Analysis = gfw.core.View.extend({
  ...
  _loadPolygon: function(the_geom) {
    ...
    this.info.model.set("ha", this._formatNumber(Math.round(ha), true));
    this.info.model.set("dataset", "protected");
