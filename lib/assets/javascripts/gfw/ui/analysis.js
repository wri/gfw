...
gfw.ui.view.AnalysisInfo = gfw.ui.view.Widget.extend({
  ...
  initialize: function() {
    ...
    this.dataset = 'umd';
    this.iso = window.location.href.split('/')[7];
...
gfw.ui.view.Analysis = gfw.core.View.extend({
  ...
  _loadPolygon: function(the_geom) {
    ...
    var polygon = the_geom.features[0]['geometry'];

    var ha = that._calcAreaMultiPolygon(polygon);
  ...
  _loadCountryGeoJSON: function(ISO) {
    ...
    var that = this;
    var query = "https://wri-01.cartodb.com/api/v2/sql?q=SELECT the_geom FROM ne_50m_admin_0_countries WHERE iso_a3 ='" + ISO + "'&format=geojson";
