  ...
  */
  ...
  _animate_action: function() {
    ...
    if (this.months[line_x]) {
      this.$line.find(".tipsy span").html(this.months[line_x][2]);
    }
  ...
  _updateMap: function(side, value) {
    ...
    if (dates) {
      // this.trigger('change_date', 2000, dates[2]);
      this.trigger('change_date', dates[0], dates[2]);
  ...
  */
  ...
  _adjustHandlePosition: function() {
    ...
    var year = _.find(this.years, function(y) {
      return (x >= (y[0] - 5) && x <= (y[0] + 27 - 5));
    });

    if (year && typeof(year[1]) == "number" ) {
      ...
      this.$current_drag.find(".tipsy span").html(this.months[month_pos][2]);

      if (this.current_drag_side == 'left') {
        this.$line.find(".tipsy span").html(this.months[month_pos][2]);
      } else {
  ...
  _updateTipsy: function(x, left_side) {
    if (this.months[x]) {
      ...
      if (left_side) {
        ...
        this.$left_tipsy.find("span").html(this.months[x][2]);
        this.$line.find(".tipsy span").html(this.months[x][2]);
        this.$left_tipsy.show().delay(2000).fadeOut(250);
      }

    } else {
      this.$left_tipsy.hide();
      this.$right_tipsy.find("span").html(this.months[x][2]);
...
_addYears: function() {
  ...
  var width = padding + (yearCount)*liWidth + 40;
...
_init: function() {
  ...
  if (this.months[left_handle_x]) {
    this.$line.find(".tipsy span").html(this.months[left_handle_x][2]);
  }

  this.$range.css({ left: this.model.get("left_handle"), width: this.model.get("right_handle") - 5*2 });
  this.$range.fadeIn(250);

  if (!showMap) this.$el.hide();
