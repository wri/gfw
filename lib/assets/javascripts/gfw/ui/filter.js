var Filter = (function() {
  ...
  // setTimeout(function() {
  //   /*AY... I KNOW, PERO*/
  //   var $links = $('.filter-links');
  //   var ifl2013 = $links.find('li.ifl_2013_deg');
  //   var ifl2000 = $links.find('li.ifl_2000');
  //   var content = '<ul><li id="filter_603_" class="forest_cover ifl_2013_deg">'+ifl2013.html() + '</li>'+
  //                     '<li id="filter_602_" class="forest_cover ifl_2000">' +ifl2000.html()+'</li></ul>';
  //   ifl2000.after(content)

  //   ifl2000.css({'position':'absolute','visibility':'none','left':'-999999px'});
  //   ifl2013.css({'position':'absolute','visibility':'none','left':'-999999px'});
  //   $('#filter_603_').find('a').on('click',function(){ ifl2013.find('a').trigger('click') });
  //   $('#filter_602_').find('a').on('click',function(){ ifl2000.find('a').trigger('click') });
  //   }, 1000);
  ...
  function _addForestLossFilter(id, slug, category, name, options) {
    ...
    if (slug === 'loss' && config.BASELAYER && config.BASELAYER.indexOf('loss') === 0 || slug === 'forestgain' && _.include(config.MAPOPTIONS.layers, 596)) {
  ...
  function _addForestLossFilters(id, slug, category, name, options) {
    ...
    if (config.BASELAYER && config.BASELAYER.indexOf('loss') === 0 || _.include(config.MAPOPTIONS.layers, 596)) {
  ...
  function _addFilter(id, slug, category, name, options) {
    ...
    if ((slug === 'nothing' && config.BASELAYER === null && !_.include(config.MAPOPTIONS.layers, 596)) || (slug === config.BASELAYER)) {
      $layerItem.find('.radio').addClass('checked');
    }
