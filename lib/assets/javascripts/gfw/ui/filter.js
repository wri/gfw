var Filter = (function() {
  ...
  function _init() {
    ...
    $(document).on('click', '.checkbox', function(e) {
      ...
      if ($(this).attr('data-id') === '591') {
        $comp_layer = $('.checkbox.umd_tree_loss_gain');
      } else if ($(e.target).closest('.checkbox').hasClass('umd_tree_loss_gain')) {
      ...
      if ($comp_layer) {
        $comp_layer.toggleClass('checked');

        if ($comp_layer.hasClass('checked')) {
          var color = $comp_layer.attr('data-color');
          $comp_layer.css('color', color);
          $comp_layer.find('i').css('background-color', color);
        ...
        if ($comp_layer && comp_layer === 'legend') {
          that.toggle(591);
        }
      }
    });
  ...
  function _check(id) {
    ...
    if (id === 591 && $('#layer a[data-id="591"]').hasClass('checked')) {
      var $comp_layer = $('.checkbox.umd_tree_loss_gain');

      $comp_layer.addClass('checked');

      var comp_color = $comp_layer.attr('data-color');
      $comp_layer.css('color', comp_color);
      $comp_layer.find('i').css('background-color', comp_color);
