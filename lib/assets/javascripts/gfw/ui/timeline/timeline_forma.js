...
gfw.ui.view.TimelineForma = gfw.ui.view.Widget.extend({
  ...
  _onMouseUpHandle: function() {
    ...
    if (x > max_x) {
      if( this.$current_drag.hasClass('left') ){
        this._updateTipsy( max_x - 3, true );
      } else {
        this._updateTipsy( max_x - 3, false );
      }
    }
  },
  ...
  _adjustHandlePosition: function() {
    ...
    if (x > max_x) {
      this.model.set('right_handle', max_x - 3);
      this.$range.width(max_x - this.$left_handle.position().left) - 3;
      x = max_x - 3;
    }
  ...
  _init: function() {
    ...
    if (url('?begin') && url('?end') && url('?begin').length > 6 && url('?end').length > 6) {
      ...
      left_handle_x  = 27;
      right_handle_x = ((8 * 108) + (parseInt(today.getUTCMonth(), 10) -1) * 16) - 3;
    }
    ...
    var a_month  = this.$years.find('a[date-m="' + month + '"][date-y="' + year + '"]'),
