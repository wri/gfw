...
gfw.ui.view.TimelineForma = gfw.ui.view.Widget.extend({
  ...
  _getDates: function(side, value) {
    ...
    if (this.months[start_pos] && this.months[end_pos]) {
      ...
      var today       = new Date();
      var month_pos   = today.getUTCFullYear().toString().substr(2,4) * 12 + parseInt(today.getUTCMonth(), 10) + 1;
      if (this.months[end_pos][2] >= today.getUTCFullYear()) {
        var month_names = ["JAN","FEB","MAR","APR","MAY","JUN","JUL","AUG","SEP","OCT","NOV","DEC"];

        if (month_names.indexOf(this.months[end_pos][0]) >= today.getUTCMonth()){
          end_month = month_pos;
          console.log(end_month, month_pos)
        }
      }
  ...
  _updateMap: function(side, value, options) {
    ...
    if (dates) {
      ...
      if (options) publish('timeline:change_date_forma', dates);
  ...
  _init: function() {
    ...
    if (url('?begin') && url('?end') && url('?begin').length > 6 && url('?end').length > 6) {
      ...
      left_handle_x  = 27;
      right_handle_x = parseInt(_.keys(this.months)[_.size(this.months)- 1], 10) || 957;
    }
