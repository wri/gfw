...
gfw.ui.view.TimelineForma = gfw.ui.view.Widget.extend({
  ...
  _onMouseUpHandle: function() {
    var x     = this.$current_drag.position().left,
        max_x = ((8 * 108) + this.max_month_forma * 16) + 7;
  ...
  _onClickYear: function(e) {
    ...
    var x = parseInt($(e.target).position().left, 10),
        max_x = ((8 * 108) + (parseInt(today.getUTCMonth(), 10) -1) * 16);
  ...
  _getMaxFormaMonth: function() {
    ...
    $.ajax({
      ...
      success: function(data) {
        return data.rows[0].max.split('-')[1];
      },
      error: function() {
        return 0;
      }
  ...
  _init: function() {
    ...
    if (url('?begin') && url('?end') && url('?begin').length > 6 && url('?end').length > 6) {
      ...
      this.max_month_forma = this._getMaxFormaMonth();
      //debugger;
      this.max_month_forma = (this.max_month_forma === 0 || this.max_month_forma === undefined) ? month -2 : (this.max_month_forma < 10) ? '0' + this.max_month_forma : this.max_month_forma;
      left_handle_x  = 27;
      right_handle_x = ((8 * 108) + this.max_month_forma * 16) + 7;
    }
