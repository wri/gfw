...
gfw.ui.view.TimelineLoss = gfw.ui.view.Widget.extend({
  ...
  _animate_action: function() {
    ...
    if (line_x + 80 >= this.model.get('right_handle')) { // one year
  ...
  _set_range_years: function(){
    this.range_year[0] = url('?begin');
    this.range_year[1] = url('?end');
  },
  ...
  _onDrag: function() {
    ...
    this._updateDate( (this.current_drag_side === 'left')? x : x + 45 );

    if ( this.current_drag_side === 'left' && current_handle_pos + this.grid_x > this.$right_handle.position().left + 45) {
      this.fixPosition = 'left';
    } else if ( this.current_drag_side === 'right' && current_handle_pos - this.grid_x < this.$left_handle.position().left - 45) {
  ...
  _highlightYears: function() {
    ...
    this.$el.find('.years li:lt(' + (min + 1) + ') .year, .years li:gt(' + (max - 1) + ') .year').removeClass('active');
  },
  ...
  _onStopDragging: function() {
    ...
    if (this.fixPosition) {
      if      (this.fixPosition === 'left')  this.model.set('left_handle',  this.model.get('right_handle') - this.grid_x + 45);
      else if (this.fixPosition === 'right') this.model.set('right_handle', this.model.get('left_handle')  + this.grid_x - 45);
    ...
    setTimeout(function() {
      ...
      $.each(that.$years.find('li .year'), function(i, el){
        ...
        if ($(el).data('year') > (year_left + 1) && $(el).data('year') < (year_right +1)) {
          $(el).addClass('active')
        }
      });
  ...
  _adjustHandlePosition: function() {
    ...
    if (this.current_drag_side === 'right') {
      pos = pos + 45;
      left = false;
    }
    this._updateRangeYears(pos, left);
  ...
  _onClickYear: function(e) {
    ...
    if (left) {
      ...
      this.model.set('right_handle', x - 45);
      this.$range.css({
        left: parseInt(this.$left_handle.position().left, 10),
        width: x - 45 - parseInt(this.$left_handle.position().left, 10)
      });
    }
  ...
  _init: function() {
    ...
    if(url('?begin') && url('?begin')){
        url_begin = (( (url('?begin') <= 2000)                       || (parseInt(url('?begin')) > parseInt(url('?end'))) )   ? 2001 : url('?begin') ).toString();
        url_end   = (( (url('?end')    > today.getUTCFullYear() - 1) || (parseInt(url('?end'))   < parseInt(url('?begin'))) ) ? today.getUTCFullYear() - 1 : url('?end') ).toString();

      if (url_begin === url_end) {
        url_end = (parseInt(url_end) + 1).toString();
        if (parseInt(url_end) >= today.getUTCFullYear() ) {
          url_end   = (2001).toString();
          url_begin = (2002).toString();
        }
      }
      if (url_begin && url_end && url_begin.length < 5 && url_end.length < 5) {
        this.range_year[0] = _.find(this.years, function(year) {
          return year == url_begin;
        });

        _.each(this.years, function(year, pos) {
          if (year == url_begin) left_handle_x = parseInt(pos, 10);
        });
        ...
        _.each(this.years, function(year, pos) {
          if (year == url_end) right_handle_x = parseInt(pos, 10);
        });
        this._highlightYears();
        this._updateDate(right_handle_x)
      }
    } else {
      left_handle_x = 30;
    ...
    this.model.set('right_handle', right_handle_x - 45);
