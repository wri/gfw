  ...
  _onStopDragging: function() {
    ...
    setTimeout(function() {
      ...
      if(real_diff !== range_diff){
        ...
        //This gives the year from the left and from the right when we input the position: 30px from the left --> year 2000.
        //(position + year 2000 (first year) - 30 (tipsy width) ) / 80 (year width) + 1975 (fill the gap)
        var year_left  = (parseInt(that.$left_handle.css('left'), 10) + 2000 - 30) / 80 + 1975,
            year_right = (parseInt(that.$right_handle.css('left'),10) + 2000 - 30) / 80 + 1975;
