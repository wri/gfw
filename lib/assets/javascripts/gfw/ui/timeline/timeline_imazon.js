...
gfw.ui.view.TimelineImazon = gfw.ui.view.Widget.extend({
  ...
  _getDates: function(side, value) {
    ...
    if (this.months[start_pos] && this.months[end_pos]) {
      ...
      var start_year  = parseInt(url('?begin').substr(0,4),10) || this.months[start_pos][2];

      var end_month   = this.first_month + this.months[end_pos][1];
      var end_year    = parseInt(url('?end').substr(0,4),10) || this.months[end_pos][2];
  ...
  _updateMap: function(side, value, options) {
    ...
    if (dates) publish('timeline:change_date_imazon', [dates[0], ((dates[1]-71)%12)+1, dates[2], ((dates[3]-71)%12)+1, options]);
  ...
  _init: function() {
    ...
    var left_handle_x  = 27,
        right_handle_x = parseInt(_.keys(this.months)[_.size(this.months)- 1], 10) || 741;
