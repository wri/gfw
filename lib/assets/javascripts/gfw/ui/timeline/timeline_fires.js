...
gfw.ui.view.TimelineFires = gfw.ui.view.Widget.extend({
  ...
  updateMap: function() {
    publish('timeline:change_date_fires', [this.model.get('month'), this.model.get('year')]);
  },
  ...
  _loadDefaultQuarter: function() {
    ...
    var today = new Date(),
        start = today.getUTCDate() - 7;
debugger;
    var daysCount = (today.getUTCDate() - start),
        quarters = 4,
        sql = 'SELECT ';

    for(var i = 0; i <= daysCount; i++) {
      sql += ['(WITH fires as (SELECT the_geom',
                              'FROM global_7d',
                              'WHERE EXTRACT(YEAR FROM acq_date) = '+ today.getUTCFullYear(),
                              'AND EXTRACT(MONTH FROM acq_date) = '+ today.getUTCMonth(),
                              'AND EXTRACT(DAY FROM acq_date) = '+ (start + i),
                              ') '
             ].join(' ');

      sql += 'SELECT COUNT(*) as d'+(start + i)+' FROM fires)';
    }
debugger;
