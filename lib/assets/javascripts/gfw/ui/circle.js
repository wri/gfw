...
gfw.ui.view.Circle = cdb.core.View.extend({
  ...
  initialize: function() {
    ...
    _.each(this.options.circles, function(circle) {
      if (circle.slug) {
        that.circles.add(new gfw.ui.model.Circle({ icon: circle.icon, count: numberForCircle(circle.num, circle.slug === 'gain'), title: circle.title }));
      } else {
        that.circles.add(new gfw.ui.model.Circle({ icon: circle.icon, title: circle.title, link: circle.link }));
      }
    });
  ...
  _onClick: function(e) {
    if (!this.currentCircle.get('link')) {
      e && e.preventDefault();
  ...
  _onMouseEnter: function() {
    ...
    if (this.animating || this.currentCircle.get('link')) return;
