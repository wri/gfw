...
gfw.ui.view.Timeline = gfw.ui.view.Widget.extend({
  ...
  getStoredMonth: function() {

    var x = this.model.get("left_handle")

    if (this.months[x]) {
      return this.first_month + this.months[x][1];
    }
  ...
  _updateMap: function(x) {
    ...
    if (this.months[x]) {
      var month_number = this.first_month + this.months[x][1];
      var year         = this.months[x][2];

      this.trigger('change_date', month_number, year);
    }
  ...
  _onDrag: function(e) {
    ...
    if (this.current_drag_side != 'left') this._updateMap(x);
