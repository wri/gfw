...
gfw.ui.view.Timeline = gfw.ui.view.Widget.extend({
  ...
  _animate: function() {
    ...
    this.animationPid = setTimeout(function() {
      ...
      var line_x = parseInt(that.model.get("line"), 10);

      if (line_x >= that.model.get("right_handle")) {
        that.model.set("line", line_x + 6 );
        that._stopAnimation();
      } else that.model.set("line", line_x + 9 );

      if (that.months[line_x]) {
        that.$line.find(".tipsy span").html(that.months[line_x][0]);
      }
      ...
      setTimeout(function() {
        ...
        console.log(that.months[line_x]);

        that._updateMap(line_x);
        that._animate();
      }, 500);
  ...
  _updateMap: function(x) {

    if (that.months[x]) {
      var month_number = 70 + that.months[x][1];
      var year = that.months[x][2];

      console.log(month_number, year);

      that.trigger('change_date', month_number, year);
    }
  ...
  _onChangePlayMode: function(e) {
    ...
    if (this.model.get("play_mode")) {

      this.$pause.fadeOut(150, function() {

        that.$line.delay(1000).fadeOut(150);
  ...
  _highlightYears: function() {
    ...
    this.$el.find(".years li:lt(" + (min + 1) + ") .year").removeClass("active");

    if ((min + 1) < max) {
      this.$el.find(".years li:gt(" + (min) + "):lt(" + (max) + ") .year").addClass("active");
    } else if (min != max) {
      this.$el.find(".years li:gt(" + (min ) + "):lt(" + (max - 1) + ") .year").addClass("active");
    }

    this.$el.find(".years li:gt(" + (max - 1) + ") .year").removeClass("active");
    this.$el.find(".years li:eq(" + (max) + ") .year").addClass("active");
  },
  ...
  _updateDate: function(x) {
    ...
    if (x && this.months[x]) {
      ...
      this.$current_drag.find(".tipsy span").html(date);
  ...
  _adjustHandlePosition: function() {
    ...
    //console.log((year && year[1] && year[1] == "number"), year, year && year[1], typeof(year[1]));
  ...
  _updateTipsy: function(x, left_side) {
    ...
    if (left_side) {
      ...
      if (this.months[x]) {
        ...
        this.$line.find("span").html(this.months[x][0]);
