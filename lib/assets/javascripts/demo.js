...
.on("mouseover", function(d) {
  ...
  $(".amount").text(val);
  d3.select(this).transition().duration(100).style("fill", hoverColor);
})
.on("mouseout", function() { d3.select(this).transition().duration(500).style("fill", color); })

}

data = d3.range(40).map(function() { return 100 * Math.random(); }),
addBars(data, { title: "Height", subtitle:"Tree height distribution", legend:"total forest area", hoverColor: "#427C8D", color: "#75ADB5", unit: "Ha" });
