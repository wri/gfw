...
define(
  ...
  (d3, UriTemplate, CanvasLayerClass, Presenter) => {
    const PantropicalLayer = CanvasLayerClass.extend({
      options: {
        threshold: 30,
        dataMaxZoom: 12,
        urlTemplate:
          'https://storage.googleapis.com/earthenginepartners-wri/whrc-hansen-carbon-{threshold}-{z}{/x}{/y}.png'
      },
      ...
      filterCanvasImgdata(imgdata, w, h) {
        ...
        for (let i = 0 | 0; i < w; ++i) {
          for (var j = 0 | 0; j < h; ++j) {
            let pixelPos = ((j * w + i) * components) | 0,
            ...
            let intensity_scaled = myscale(intensity) | 0,
