...
define([
  ...
  var FiresLayer = CartoDBLayerClass.extend({
    options: {
      sql: 'SELECT the_geom_webmercator, acq_time,  COALESCE(to_char(acq_date, \'MM-DD-YYYY\')) as acq_date, confidence, brightness, longitude, latitude FROM global_7d WHERE acq_date > \'{year}-{month}-{day}\' AND CAST(confidence AS INT) > 30',
