...
define(
  ...
  (_, CartoDBLayerClass, Places2WatchCartocss) => {
    const Places2WatchLayer = CartoDBLayerClass.extend({
      options: {
        sql:
      ...
      ptwInfowindowsUIEvents(model) {
        const $map = $('#map');
        $map
          .find('.cartodb-popup')
          .on('click.infowindow', '.js-ptw-track', function (e) {
            const target = $(e.target);
            const ptwKey = target.attr('data-ptw-key');
