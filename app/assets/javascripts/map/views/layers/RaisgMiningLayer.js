 ...
 */
define(['uri', 'abstract/layer/WMSLayerClass'], (
  UriTemplate,
  WMSLayerClass
) => {
  let wmsUrl =
    'https://geo.socioambiental.org/webadaptor2/services/raisg/raisg_mineria/MapServer/WMSServer?';
  wmsUrl += '&SERVICE=WMS';
  ...
  wmsUrl += '&WIDTH=256';
  wmsUrl +=
    '&SLD=https://gfw-nav.herokuapp.com/assets/map/cartocss/raisg_mining.xml';
  wmsUrl += '&CRS=EPSG%3A3857';

  const wmsInfowindowUrl =
    'https://geo.socioambiental.org/webadaptor2/services/raisg/raisg_mineria/MapServer/WMSServer/1/query?f=json&where=1%3D1&returnGeometry=false&spatialRel=esriSpatialRelIntersects&outFields=pais,nombre,cia,situacion,tipo_minerio,fuente,area_oficial_ha,leyenda,grupo&geometryType=esriGeometryPoint&geometry={longitude},{latitude}';

  const RaisgMiningLayer = WMSLayerClass.extend({
    options: {
      url: wmsUrl,
      infowindowUrl: wmsInfowindowUrl
    },

    getQuery(_longitude, _latitude, _bbox) {
      return new UriTemplate(wmsInfowindowUrl).fillFromObject({
