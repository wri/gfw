...
define([
  ...
  var LossByDriverLayer = CanvasLayerClass.extend({
    ...
    options: {
      threshold: 30,
      dataMaxZoom: 12,
      urlTemplate: 'https://storage.googleapis.com/wri-public/lossyear_classification_map/2017/gfw/tiles/hansen_world/v2/tc{threshold}{/z}{/x}{/y}.png',
      ...
      for(var i = 0; i < w; ++i) {
        for(var j = 0; j < h; ++j) {
          ...
          if (lossCat === 1) {
            ...
            rgb = [100, 100, 100];  //zero or minor loss 
          }

          if (yearLoss >= yearStart && yearLoss < yearEnd) {
            imgdata[pixelPos] = rgb[0];
            imgdata[pixelPos + 1] = (72 - z) + rgb[1] - (3 * myscale(intensity) / z);
