    ...
    var AnalysisView = Backbone.View.extend({
        ...
        var handleUpload = function(file) {
          ...
          shapeService.toGeoJSON().then(
            function(data) {
              ...
              var bounds = geojsonUtilsHelper.getBoundsFromGeojson(
                combinedFeatures
              );
      ...
      getCountries: function() {
        if (!amplify.store('countries')) {
          var sql = ['SELECT c.iso, c.name_0 FROM gadm36_countries c'];
          $.ajax({
      ...
      getSubCountries: function() {
        ...
        var sql = [
          "SELECT gadm36_adm1.cartodb_id, gadm36_adm1.iso, gadm36_adm1.gid_1, gadm36_adm1.name_1 as name_1 FROM gadm36_adm1 where gadm36_adm1.iso = '" +
            this.iso +
            "' ORDER BY gid_1 asc"
        ];
        $.ajax({
      ...
      drawMaskArea: function(geojson, iso, region) {
        this.mask = cartodb.createLayer(this.map, {
          ...
          sublayers: [
            {
              sql: 'SELECT * FROM gadm26_countries',
              cartocss:
                "\
            #country_mask {\
              polygon-fill: #373737;\
            ...
            {
              sql: "SELECT * FROM gadm36_adm1 WHERE iso LIKE '" + iso + "' ",
              cartocss:
                '\
            #gadm36_adm1 {\
              polygon-fill: #373737;\
