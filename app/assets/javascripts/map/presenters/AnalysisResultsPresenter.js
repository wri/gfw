...
define([
  ...
  var AnalysisResultsPresenter = PresenterClass.extend({
    ...
    _getAnalysisResource: function(results, layer) {
       ...
       */
      if (layer.slug === 'loss') {
        p.lossDateRange = '{0}-{1}'.format(dateRange[0].year(), dateRange[1].year()); // '+1' --> in case the user selects 2010, the analysis scope should end at beginning of 2011, HARDCODED
        p.lossAlerts = 0;
        p.gainAlerts = 0;
