...
gfw.ui.view.CountryHeader = cdb.core.View.extend({
  ...
  _displayArea: function(area) {
    var self = this;
    this.$el.find('#url_analysis').attr('href', '/map/6/'+ area.attributes.bounds._northEast.lat + '/'+ area.attributes.bounds._northEast.lng + '/'+ area.attributes.iso + '-' + String(area.get('id_1')) +'/grayscale/loss,forestgain?begin=&end=&threshold='+(config.canopy_choice || 10));
