  ...
  Indicator.prototype = {
    ...
    _end(e) {
      ...
      if (this.scroller.options.snap) {
        const snap = this.scroller._nearestSnap(
          this.scroller.x,
          this.scroller.y
        );
...
(function(window, document) {
  window.fp_scrolloverflow = (function() {
    ...
    function getPaddings(element) {
      const section = fp_utils.closest(element, SECTION_SEL);
      if (section != null) {
        return (
    ...
    function scrollBarHandler() {
      ...
      function createScrollBar(element) {
        ...
        const scrollOverflowHandler = self.options.scrollOverflowHandler;
        const wrap = scrollOverflowHandler.wrapContent();
        const section = fp_utils.closest(element, SECTION_SEL); // in case element is a slide
        const scrollable = scrollOverflowHandler.scrollable(element);
        let contentHeight;
        const paddings = getPaddings(section);
        ...
        const scrollHeight = fp_utils.getWindowHeight() - paddings;
      ...
      function removeResponsiveScrollOverflows() {
        const scrollOverflowHandler = self.options.scrollOverflowHandler;
        forEachSectionAndSlide(element => {
          if (
            fp_utils.hasClass(
    ...
    var iscrollHandler = {
      ...
      toggleWheel(value) {
        const scrollable = $(SCROLLABLE_SEL, $(SECTION_ACTIVE_SEL)[0]);
        scrollable.forEach(item => {
          const iScrollInstance = item.fp_iscrollInstance;
       ...
       */
      create(element, scrollHeight, iscrollOptions) {
        const scrollable = $(SCROLLABLE_SEL, element);

        scrollable.forEach(item => {
          fp_utils.css(item, { height: `${scrollHeight}px` });

          let iScrollInstance = item.fp_iscrollInstance;
          if (iScrollInstance != null) {
            iscrollHandler.iScrollInstances.forEach(instance => {
              instance.destroy();
            });
          }
      ...
      isScrolled(type, scrollable) {
        const scroller = scrollable.fp_iscrollInstance;
      ...
      remove(element) {
        ...
        const scrollable = $(SCROLLABLE_SEL, element)[0];
        if (scrollable != null) {
          const iScrollInstance = scrollable.fp_iscrollInstance;
          if (iScrollInstance != null) {
